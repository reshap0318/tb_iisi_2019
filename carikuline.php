<?php
	...
	$querysearch="SELECT id, name, address, cp, capacity, open, close, employee, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat,
	...
	FROM culinary_place where CAST(ST_DistanceSpheroid(ST_GeomFromText('POINT($longi $latit)',-1),ST_Centroid(geom),'SPHEROID[\"WGS 84\",6378137,298.257223563]') As numeric) <= ".$rad."";
